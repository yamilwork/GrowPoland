...
const Home: React.FC<HomeProps> = ({ content, onNavigateToArticle, onNavigateToBrand }) => {
  ...
  return (
    <div className="w-full overflow-x-hidden">
            ...
            <div className="lg:w-1/2">
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div>
                  ...
                  <p className="text-sm text-slate-500">{content.about.mission}</p>
                </div>
                <div>
                  <h4 className="font-bold text-slate-900 mb-2 flex items-center">
                    <span className="w-2 h-2 bg-slate-400 rounded-full mr-2"></span> {content.about.visionLabel}
                  </h4>
                  <p className="text-sm text-slate-500">{content.about.vision}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="lg:w-1/3 flex flex-col items-center">
              <div className="bg-white text-slate-900 p-8 md:p-12 rounded-[2.5rem] shadow-2xl text-center transform lg:rotate-6 hover:rotate-0 transition-all duration-500 border border-slate-100">
                ...
                <button 
                  ...
                  className="w-full py-4 bg-red-600 text-white font-bold rounded-2xl uppercase text-[10px] tracking-[0.2em] hover:bg-red-700 transition-all shadow-xl hover:shadow-red-900/40"
                >
            ...
            <div className="grid md:grid-cols-2 gap-20">
              ...
              <div className="bg-white text-slate-900 p-10 rounded-2xl shadow-2xl">
                <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
                  ...
                  <button className="w-full py-4 bg-slate-900 text-white font-bold rounded uppercase text-xs tracking-widest hover:bg-red-600 transition-all shadow-lg">
      ...
      {/* Supplier Modal */}
      {isSupplierModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl transition-all animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] p-8 md:p-12 max-w-xl w-full shadow-2xl relative animate-in zoom-in duration-300 border border-slate-100">
            <button 
            ...
            <div className="mb-8">
              ...
              <h3 className="text-3xl font-black text-slate-900 leading-tight mb-4">{content.exportDivision.modalTitle}</h3>
            ...
            <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); setIsSupplierModalOpen(false); }}>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input type="text" required placeholder={content.contact.form.name} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm focus:outline-none focus:ring-4 focus:ring-red-600/5 focus:border-red-600 transition-all" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{content.contact.form.company}</label>
                  <input type="text" required placeholder={content.contact.form.company} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm focus:outline-none focus:ring-4 focus:ring-red-600/5 focus:border-red-600 transition-all" />
                </div>
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{content.contact.form.email}</label>
                <input type="email" required placeholder={content.contact.form.email} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm focus:outline-none focus:ring-4 focus:ring-red-600/5 focus:border-red-600 transition-all" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{content.exportDivision.formLabelProduct}</label>
                <textarea rows={3} required className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm focus:outline-none focus:ring-4 focus:ring-red-600/5 focus:border-red-600 transition-all" placeholder={content.exportDivision.formPlaceholderProduct}></textarea>
              </div>
              <button className="w-full py-5 mt-4 bg-red-600 text-white font-bold rounded-2xl uppercase text-xs tracking-[0.2em] hover:bg-red-700 transition-all shadow-xl hover:shadow-red-600/40">
                {content.contact.form.submit}
              </button>
